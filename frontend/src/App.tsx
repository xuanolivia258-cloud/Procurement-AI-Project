...
export function normalizeDirectoryQuery(keyword: string) {
  const trimmed = keyword.trim();
  return /^(?:[A-Za-z]\s*)?\d(?:\s*\d)*$/.test(trimmed) ? trimmed.replace(/\s+/g, '').toLowerCase() : trimmed;
}
export async function searchDirectory(keyword: string, actorId: string): Promise<DirectoryPerson[]> {
  const response = await fetch(`${directoryUrl}?userInfo=${encodeURIComponent(normalizeDirectoryQuery(keyword))}`, { credentials: 'include', headers: { 'x-user-name': actorId } });
...
function PermissionManagement({ language, actor }: { language: Language; actor: Actor }) {
  ...
  return <section className="page permission-page"><div className="page-heading"><div><p className="eyebrow">{zh ? '访问控制' : 'ACCESS CONTROL'}</p><h1>{copy[language].settings}</h1><p>{zh ? '管理员拥有全部权限；组员只能查看和管理自己创建的项目。' : 'Administrators have full access. Members can only view and manage projects they created.'}</p></div></div>
    <div className="panel people-search"><h2>{zh ? '添加人员' : 'Add person'}</h2><div className="people-search-form"><label><span>{zh ? '姓名 / 工号' : 'Name / Employee ID'}</span><input value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && runSearch()}/></label><button className="button primary" disabled={!keyword.trim() || searching} onClick={runSearch}>{searching ? (zh ? '查询中…' : 'Searching…') : (zh ? '查询' : 'Search')}</button></div>{searchError ? <ErrorBox error={searchError} language={language}/> : null}<div className="people-results">{results.map((person) => <article key={person.w3Name}><img src={directoryAvatarUrl(person.w3Name)} alt=""/><div><strong>{zh ? person.cnName || person.fullName : person.fullName || person.cnName}</strong><span>{person.w3Name} · {person.dptName || '—'}</span></div><button onClick={() => save.mutate({ person, role: 'member' })}>{zh ? '设为组员' : 'Add member'}</button><button onClick={() => save.mutate({ person, role: 'admin' })}>{zh ? '设为管理员' : 'Add admin'}</button></article>)}</div></div>
